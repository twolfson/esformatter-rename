...
exports.transform = function (ast) {
  ...
  rocambole.moonwalk(ast, function updateIdentifiers (node) {
    ...
    if (node.type === 'Identifier' && node.scopeInfo) {
      ...
      // If the identifier is top level and we aren't touching top level items
      ...
      if (isUndeclared) {
        ...
        if (renameOptions.renameUndeclared !== true) {
          console.warn('Saw matching undeclared variable "' + name + '" but did not rename due ' +
      ...
      if (usedInAWith) {
        ...
        if (renameOptions.ignoreWith !== true) {
          console.warn('Saw matching variable "' + name + '" that was used in a `with` but did not rename due ' +
